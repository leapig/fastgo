<script lang="ts">
export default { name: 'OperatingSystemAuthRoleGroup' }
</script>
...
import {
  ...
  updateRoleGroup,
  removeRoleGroup,
  removeRoleGroupPermission
} from '@/api/operating/system/auth/roleGroup'
...
const data = reactive({
  ...
  rules: {
    role_group_name: [{ required: true, message: '请输入角色组名称', trigger: 'blur' }]
  },
  addPermission: {
    listIndex: -1,
    type: '' as any,
...
function getList() {
  ...
  getRoleGroupList(queryParams.value).then((response: any) => {
...
function handleMoreUsers(row: any) {
  addUser.value.roleGroup = objectToUnderscore(row)
...
function confirmForm() {
  proxy.$refs['formRef'].validate((valid: boolean) => {
    if (valid) {
      ...
      if (form.value.pk) {
        updateRoleGroup(form.value).then(() => {
        ...
        createRoleGroup(form.value).then(() => {
...
function handleRemove(row: any) {
  if (row.pk) {
    ElMessageBox.confirm(`确定删除角色组（${row.roleGroupName}）吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      loading.value = true
      removeRoleGroup(row.pk).then(() => {
...
function handleAddPermission(type: string, index: number) {
  ...
  addPermission.value.roleGroup = objectToUnderscore(list.value[index])
  showAddPermission.value = true
}
/** 列表移除角色/权限提交 */
function tableRemovePermissionSubmit(groupName: string, type: string, item: any) {
  if (item && item.relationPk) {
    ...
    ElMessageBox.confirm(`确定移除角色组（${groupName}）与${permissionName}的关联吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      loading.value = true
      removeRoleGroupPermission(item.relationPk).then(() => {
...
<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" inline v-show="showSearch" label-width="70px" @submit.prevent>
      <el-form-item label="名称" prop="role_group_name">
        <el-input v-model="queryParams.role_group_name" placeholder="请输入角色组名称" clearable style="width: 220px" @keyup.enter="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="list" row-key="pk" :expand-row-keys="expandRowKeys" @row-click="rowClick" @expand-change="expandListRow">
      ...
      <el-table-column label="名称" width="260" prop="roleGroupName" show-overflow-tooltip />
      <el-table-column label="用户">
        <template #default="scope">
          ...
          <el-button type="primary" circle @click.stop="handleMoreUsers(scope.row)">
              ...
              <el-descriptions-item label="角色：">
                <template v-for="role in scope.row.roleRows" :key="role.pk">
                  <el-tag
                    ...
                    @close="tableRemovePermissionSubmit(scope.row.roleGroupName, 'role', role)"
              ...
              <el-descriptions-item label="权限组：">
                <template v-for="permissionGroup in scope.row.permissionGroupRows" :key="permissionGroup.pk">
                  <el-tag
                    ...
                    class="desc-tag"
                    :closable="permissionStore.hasWritePermi()"
                    @close="tableRemovePermissionSubmit(scope.row.roleGroupName, 'permission_group', permissionGroup)"
              ...
              <el-descriptions-item label="权限：">
                <template v-for="permission in scope.row.permissionRows" :key="permission.pk">
                  <el-tag
                    ...
                    class="desc-tag"
                    :closable="permissionStore.hasWritePermi()"
                    @close="tableRemovePermissionSubmit(scope.row.roleGroupName, 'permission', permission)"
    ...
    <el-drawer v-model="showForm" :size="500" append-to-body>
      <template #header>
        <h4>{{ form.pk ? '修改' : '新增' }}角色组</h4>
      </template>
      <template #default>
        <div class="drawer-content">
          <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
            <el-form-item label="名称" prop="role_group_name">
              <el-input v-model="form.role_group_name" placeholder="请输入角色组名称" style="width: 300px" />
            </el-form-item>
      ...
      <template #footer>
        <div style="flex: auto">
          ...
          <el-button type="primary" :loading="formLoading" @click="confirmForm" v-writePermi>
    ...
    <operating-system-auth-permission-add-permission
      ...
      :data="addPermission.roleGroup"
      name-key="role_group_name"
