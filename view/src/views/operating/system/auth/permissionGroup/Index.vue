<script lang="ts">
export default { name: 'OperatingSystemAuthPermissionGroup' }
</script>
...
import {
  ...
  removePermissionGroupPermission
} from '@/api/operating/system/auth/permissionGroup'
...
const { queryParams, form, rules, groupTypeOptions, addPermission } = toRefs(data)
...
function getList() {
  ...
  getPermissionGroupList(queryParams.value).then((response: any) => {
    response.data.rows.forEach((item: any) => {
      item.permissionRows = item.rows
    })
...
function confirmForm() {
  proxy.$refs['formRef'].validate((valid: boolean) => {
    if (valid) {
      ...
      if (form.value.pk) {
        updatePermissionGroup(form.value).then(() => {
        ...
        createPermissionGroup(form.value).then(() => {
...
function handleRemove(row: any) {
  if (row.pk) {
    ElMessageBox.confirm(`确定删除权限组（${row.groupName}）吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      loading.value = true
      removePermissionGroup(row.pk).then(() => {
...
function handleAddPermission(type: string, index: number) {
  ...
  addPermission.value.permissionGroup = objectToUnderscore(list.value[index])
...
function tableRemovePermissionSubmit(groupName: string, type: string, item: any) {
  if (item && item.relationPk) {
    ...
    ElMessageBox.confirm(`确定移除权限组（${groupName}）与${permissionName}的关联吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      loading.value = true
      removePermissionGroupPermission(item.relationPk).then(() => {
  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" inline v-show="showSearch" label-width="70px" @submit.prevent>
      <el-form-item label="名称" prop="group_name">
        <el-input v-model="queryParams.group_name" placeholder="请输入权限组名称" clearable style="width: 220px" @keyup.enter="handleQuery" />
      </el-form-item>
      <el-form-item label="所属平台" prop="group_type">
              ...
              <el-descriptions-item label="权限：">
                <template v-for="permission in scope.row.permissionRows" :key="permission.pk">
                  <el-tag
                    ...
                    @close="tableRemovePermissionSubmit(scope.row.groupName, 'permission', permission)"
    ...
    <el-drawer v-model="showForm" :size="500" append-to-body>
      <template #header>
        <h4>{{ form.pk ? '修改' : '新增' }}权限组</h4>
      </template>
      <template #default>
        <div class="drawer-content">
          <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
            <el-form-item label="名称" prop="group_name">
              <el-input v-model="form.group_name" placeholder="请输入权限组名称" style="width: 300px" />
            </el-form-item>
            <el-form-item label="所属平台" prop="group_type">
              <el-select v-model="form.group_type" placeholder="请选择所属平台" style="width: 300px">
                <el-option v-for="item in groupTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
    ...
    <operating-system-auth-permission-add-permission
      ...
      :data="addPermission.permissionGroup"
      name-key="group_name"
